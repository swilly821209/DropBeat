...
export default {
  ...
  methods: {
    ...
    sendData () {
      if (this.$store.getters.loginState !== false) {
        ...
        fetch('./DropbeatBackend/FileUpload/activity_files_send.php', {
          method: 'POST',
          body: form
        })
        // 其他前台動作
    ...
    async edidFund (e) {
      ...
      const response = await fetch('./DropbeatBackend/FileUpload/activity_files_edit_get.php', {
        method: 'POST',
        body: form
      })
    ...
    sendEdit () {
      ...
      fetch('./DropbeatBackend/FileUpload/activity_files_edit_send.php', {
        method: 'POST',
        body: form
      })
      this.$router.replace('/Active')
    ...
    deleteFun () {
      ...
      fetch('./DropbeatBackend/FileUpload/activity_delete.php', {
        method: 'POST',
        body: form
      })
      this.deleteDialog = !this.deleteDialog
  ...
  async created () {
    ...
    if (this.$store.getters.memberIdState) {
      ...
      const response = await fetch('./DropbeatBackend/FileUpload/activity_files_get.php', {
        method: 'POST',
        body: form
      })
      // 回傳
