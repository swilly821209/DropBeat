<template>
  <div class="sm:flex sm:w-[800px] sm:min-w-[800px] sm:justify-between">
    ...
    <div class="text-xs space-x-2">
      <button @click="release" class="px-2 py-[2px] rounded-2xl bg-orange text-white hover:bg-blue-light">發佈</button>
      <button class="px-2 py-[2px] border-current text-gray-light hover:text-orange border rounded-2xl">暫存草稿</button>
    ...
    <div class="text-black-backdrop flex-col flex h-52 justify-evenly sm:h-64">
      ...
      <label for="file" class="block">
          <div class="text-gray-light flex" >
            ...
            <div class="bg-center bg-no-repeat bg-cover w-20 h-20 sm:w-36 sm:h-36  bg-gray-light flex flex-col justify-center items-center">
    ...
    <div>
      ...
      <swiper :slidesPerView="5" :direction="'vertical'" :navigation="{nextEl: '.nextArrow', prevEl: '.preArrow'}" class="h-[340px] sm:w-96">
        ...
        <swiper-slide v-for="item in musicsData" :key="item" class=" w-[325px] flex items-center justify-between">
          ...
          <div>
            <svg @click="toggle(item)" :class="{'text-orange': chouse.includes(item)}" class="w-5 h-5 fill-current text-gray-light" id="圖層_1" data-name="圖層 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><defs></defs><title>addmusic_d83</title><path id="Icon_awesome-plus" data-name="Icon awesome-plus" class="cls-1" d="M18.57,7.86H12.14V1.43A1.43,1.43,0,0,0,10.71,0H9.29A1.43,1.43,0,0,0,7.86,1.43h0V7.86H1.43A1.43,1.43,0,0,0,0,9.29v1.43a1.43,1.43,0,0,0,1.43,1.42H7.86v6.43A1.43,1.43,0,0,0,9.29,20h1.43a1.43,1.43,0,0,0,1.42-1.43h0V12.14h6.43A1.43,1.43,0,0,0,20,10.71V9.29a1.43,1.43,0,0,0-1.43-1.43Z"/></svg>
          </div>
        </swiper-slide>
      </swiper>
    </div>
