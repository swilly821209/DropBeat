<template>
  <div class="flex flex-col  justify-start  sm:flex-row sm:justify-between py-2 sm:py-0 px-0 mx-6 sm:m-5">
    <div class=" flex flex-col sm:mr-8 ">
      ...
      <div class="mt-0 space-x-2 order-3 sm:order-1">
        <button @click="release(1)" class="h-[25px] w-[50px] text-sm text-white bg-orange border-2 border-orange rounded-2xl hover:bg-blue-light hover:border-blue-light">發佈</button>
        <button @click="release(0)" class="h-[25px] w-[80px] text-sm text-gray-dark border-2 border-gray-default rounded-2xl  hover:text-orange hover:border-orange">暫存草稿</button>
        <button @click="close" class="h-[25px] w-[50px] text-sm text-gray-dark border-2 border-gray-default rounded-2xl  hover:text-orange hover:border-orange">離開</button>
      </div>
      <div class="space-y-3 sm:space-y-4 order-4 sm:order-2">
        <div class="text-base text-gray-dark mt-5">
          <label for="album-title" class="text-black-backdrop">專輯標題： </label>
          <input v-model="albumName" type="text" id="album-title" class="w-48 sm:w-52 text-base border-b border-gray-light focus:outline-none focus:border-orange focus:border-b-2">
        </div>
        <div class="text-base text-gray-dark">
          <label for="album-time" class="text-black-backdrop">發行時間： </label>
          <input v-model="albumDate" type="date" id="album-time" class="w-48 sm:w-52 text-base border-b border-gray-light focus:outline-none focus:border-orange focus:border-b-2">
        </div>
        <label for="file" class="block">
            <div class="text-gray-light flex" >
              <input type="file" id="file" class="hidden" @change="getFile">
              <p class="text-base text-black-backdrop mr-1">專輯照片： </p>
    ...
    <div>
      <h3 class="sm:mb-4 mb-2 mt-4 sm:mt-0 text-base text-black-backdrop" >音樂清單：</h3>
      <swiper :slidesPerView="3" :spaceBetween="20" :breakpoints='{
      ...
      }' :direction="'vertical'" :navigation="{nextEl: '.nextArrow', prevEl: '.preArrow'}" class="h-[180px] sm:h-[330px] sm:w-96">
        <div class="right-0 absolute z-50 top-16 sm:top-36 space-y-5">
        ...
        <swiper-slide v-for="(item, index) in musicsData" :key="index" :virtualIndex="index" class="w-5/6 sm:w-[325px] flex items-center justify-between">
          <div class="flex items-center w-48 sm:w-64 justify-between">
            <p class="text-2xl text-gray-dark w-[28px] text-right">{{index+1}}</p>
            <img class=" w-14 h-14" :src="item.musicImg">
            <h4 class="text-base text-black-backdrop w-20 sm:w-36 truncate">{{ item.musicName}}</h4>
          </div>
          <div>
...
export default {
  ...
  async created () {
    ...
    if (this.edit) {
      const editAlbum = await fetch('./DropbeatBackend/editAlbum.php', {
        method: 'POST',
        body: form
      })
      ...
      const musics = await fetch('./DropbeatBackend/MemberMusic.php', {
        method: 'POST',
        body: form
      })
  ...
  methods: {
    ...
    async  release (publish) {
      ...
      if (this.edit) {
        await fetch('./DropbeatBackend/SetEditAlbum.php', {
          method: 'POST',
          body: form
        })
        if (typeof this.albumPhoto !== 'string') {
          await fetch('./DropbeatBackend/changeAlbumImg.php', {
            method: 'POST',
            body: form
          })
        }
      } else {
        await fetch('./DropbeatBackend/SetAlbum.php', {
          method: 'POST',
          body: form
        })
      }
